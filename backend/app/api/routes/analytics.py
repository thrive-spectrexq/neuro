...
from app.schemas.analytics import ActivityItemResponse, SystemStatsResponse
...
async def get_system_stats(
    ...
    # Total counts
    notes_cnt = (
        await session.execute(select(func.count(Note.id)).where(Note.user_id == user_id, Note.is_archived.is_(False)))
    ).scalar() or 0
    projects_cnt = (
        await session.execute(select(func.count(Project.id)).where(Project.user_id == user_id))
    ).scalar() or 0
    ...
    notes_res = await session.execute(
        select(Note.content_type, func.count(Note.id)).where(Note.user_id == user_id).group_by(Note.content_type)
    )
