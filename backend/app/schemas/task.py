...
class TaskCreate(BaseModel):
    title: str = Field(..., min_length=1, max_length=255, description="The title of the task", examples=["Write documentation"])
    description: str | None = Field(default=None, description="Detailed description of the task", examples=["Draft the API docs for the new endpoints"])
    status: str = Field(default="todo", description="Status of the task: todo, in_progress, done", examples=["todo"])
    priority: str | None = Field(default="medium", description="Priority of the task: low, medium, high", examples=["high"])
    due_date: datetime | None = Field(default=None, description="When the task is due", examples=["2026-08-01T12:00:00Z"])
