...
async def test_list_tags(test_client: AsyncClient, auth_headers: dict[str, str]):
    await test_client.post("/api/v1/tags", json={"name": "List Tag"}, headers=auth_headers)
...
async def test_delete_tag(test_client: AsyncClient, auth_headers: dict[str, str]):
    create_resp = await test_client.post("/api/v1/tags", json={"name": "Delete Tag"}, headers=auth_headers)
